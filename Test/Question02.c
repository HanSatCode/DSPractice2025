...
typedef enum { // not 적으면 예약어라서.. 앞을 대문자 처리
    lparen, rparen,
    plus, minus, times, divide, mod, power, Not, eos,
    operand, err_code
} precedence;

static int isp[] = { 0, 19, 12, 12, 13, 13, 13, 14, 15, 0 }; // 있을 때 우선순위
static int icp[] = { 20, 19, 12, 12, 13, 13, 13, 14, 15, 0 }; // 들어올 때 우선순위
...
char printToken(precedence token) { // 토큰 출력
    ...
    case precedence::power: return '^';
    case precedence::Not: return '!';
    case precedence::eos: return 'e';
    default: return '?';
    }
}
        ...
        if (token == operand) { // 토큰이 숫자라면
            ...
            while (true) {
                ...
                if (isp[op] < icp[token]) { // 만약 넣을려고 하는 연산자가 우선순위가 더 높다면
                    stack.push(op); // 다시 넣고, 높은 연산자를 먼저 넣음
                    break;
                }
                new_expr[newExprIdx++] = printToken(op);
            }
...
int eval(char* expr) {
    ...
    while (token != eos) { // 토큰이 eos가 아닐 때까지 (스페이스 나올때까지)
        if (token == operand) { // 토큰이 숫자라면 -> (new) 2자리도 받을 수 있게 처리하기
...
int main(void) {
    printf("[infix 입력] ");
    char expr[MAX_TERMS]; scanf("%s", expr);
    expr[strlen(expr)] = '\0'; // 문자열의 끝

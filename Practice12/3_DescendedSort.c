...
void selectionSortRev(float[], int);
...
void main(void) {
	...
	for (int i = 0; i < 4; i++) { // 4번 시도
		...
		selectionSortRev(A, n); // 사전 내림차순 정렬
...
void selectionSortRev(float list[], int n) {
	int i, j, max; float temp;
	for (i = 0; i < n - 1; i++) {
		max = i;
		for (j = i + 1; j < n; j++)
			if (list[j] > list[max])
...
void checkerRev(float list[], int n) {
	int i;
	for (i = 0; i < n - 1; i++) {
		if (list[i] < list[i + 1]) {
			printf("[경고] 정렬에 실패하였습니다...\n");
